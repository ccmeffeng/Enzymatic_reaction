...
class ReactionData():
    def __init__(self, st ='training'):
        if st == 'training':
            ...
            self.data = np.load('test_set.npy').tolist()
    ...
    def next(self, batch_size):
        ...
        return batch_data, batch_labels, [462] * len(batch_seqlen)
...
# 网络参数
seq_max_len = 462 # Sequence max length
...
with tf.Session() as sess:
    ...
    for step in range(1, 2):
        ...
        if step % display_step == 0 or step == 1:
            ...
            # Calculate accuracy
            test_seqlen, test_label, test_data = [462] * len(testset.labels), testset.labels, testset.data
