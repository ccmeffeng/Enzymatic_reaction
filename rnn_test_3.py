...
class ReactionData():
    ...
    def next(self, batch_size):
        ...
        batch_seqlen = ([462] * len(batch_labels))
        # 数据集已经按长度从小到大排序，因此都按照最长的那个
        # 一个batch里按照最长的长度补齐0.0
        self.batch_id = min(self.batch_id + batch_size, len(self.data))
        return batch_data, batch_labels, [150] * len(batch_seqlen)
...
# 网络参数
seq_max_len = 150 # Sequence max length
...
with tf.Session() as sess:
    ...
    for step in range(1, training_steps+1):
        ...
        if step % display_step == 0 or step == 1:
            ...
            val_seqlen, val_label, val_data = [150] * len(valset.labels), valset.labels, valset.data
            print("Testing Accuracy:", sess.run(accuracy, feed_dict={x: val_data, y: val_label, seqlen: val_seqlen}))

    print("Optimization Finished!")

    # Calculate accuracy
    test_seqlen, test_label, test_data = [150] * len(testset.labels), testset.labels, testset.data
