...
tf.app.flags.DEFINE_integer("symbols", 30, "vocabulary size.")
tf.app.flags.DEFINE_integer("labels", 7, "Number of labels.")
tf.app.flags.DEFINE_integer("epoch", 1000, "Number of epoch.")
tf.app.flags.DEFINE_integer("embed_units", 1, "Size of word embedding.")
tf.app.flags.DEFINE_integer("units", 128, "Size of each model layer.")
...
def build_vocab(path, data):
    ...
    with open('vector.txt') as f:
        for idx, line in enumerate(f):
            ...
            word = tmp[0]
            vector = [float(tmp[1])]
            word2vector_dict[word] = vector
    for word in vocab_list:
        if word in word2vector_dict:
            embed.append(word2vector_dict[word])
        else:
            embed.append(np.zeros(FLAGS.embed_units))
...
with tf.Session(config=config) as sess:
    if FLAGS.is_train:
        ...
        while model.epoch.eval() < FLAGS.epoch:
            ...
            print("        best test accuracy [%.8f]" % best_test_acc)
